...
from movies.models import  Post as p
...
def predict():
    ...
    genre_encoder = joblib.load("Pickled_Files/genre_encoder.pkl")
    ...
    if request.method == "POST":
        ...
        #genre = genre_encoder.transform(genre)
        ...
        data = [rtime,votes,genre,years,new_budget]
        df = pd.DataFrame(data=[data], columns=['rtime', 'votes', 'genre','years','budget'])
    ...
    return render_template('index1.html', title='Predict')
        
admin.add_view(ModelView(User, db.session ))
admin.add_view(ModelView(p, db.session ))
